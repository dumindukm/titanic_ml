...
rf = RandomForestClassifier(bootstrap=True, n_estimators=1000, random_state=42,
                            max_depth=80, max_features=3, min_samples_leaf=5, min_samples_split=8)
...
def select_important_random_forest():
    importances = pd.DataFrame(
        {'feature': X.columns, 'importance': np.round(rf.feature_importances_, 3)})
    importances = importances.sort_values(
        'importance', ascending=False).set_index('feature')
    print('select_important_random_forest')
    print(importances.head(15))


def selct_best_features_from_model(model):
    sfm = SelectFromModel(model, threshold=0.15)
    # Train the selector
    sfm.fit(X, Y)
    for feature_list_index in sfm.get_support(indices=True):
        print(titanic_dataset.columns[feature_list_index])
...
def best_model_parameters(rf_model):
